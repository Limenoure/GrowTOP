...
std::string Utils::HashPassword(const std::string& username, const std::string& password) 
{
	std::string normalizedInput = username + "::" + password;
	return CreateMD5(normalizedInput);
}

std::string Utils::CreateMD5(const std::string& input) 
{
	MD5_CTX md5Context;
	MD5Init(&md5Context);
	MD5Update(&md5Context, (unsigned char*)input.c_str(), (unsigned int)input.length());
...
/*std::string Utils::SHA256Crypt(std::string input)
{
	std::uint8_t hash[EVP_MAX_MD_SIZE];
	const EVP_MD* md = EVP_sha256();
	std::uint32_t md_len;

	EVP_Digest(input.c_str(), input.size(), hash, &md_len, md, nullptr);

	std::ostringstream ss;
	for (std::uint32_t i = 0; i < md_len; ++i)
		ss << std::hex << std::setfill('0') << std::setw(2) << static_cast<std::uint32_t>(hash[i]);

	return ss.str();
}

std::string Utils::MD5Crypt(std::string input)
{
	unsigned char digest[EVP_MAX_MD_SIZE];
	unsigned int digest_len;
	EVP_MD_CTX* pCtx = EVP_MD_CTX_new();
	EVP_DigestInit_ex(pCtx, EVP_md5(), nullptr);
	EVP_DigestUpdate(pCtx, input.c_str(), input.length());
	EVP_DigestFinal_ex(pCtx, digest, &digest_len);
...
std::string Utils::GenerateRid(const std::string& game_version, const int& protocol, const std::string& rid) 
{
	...
	{
			SHA256Crypt(MD5Crypt(SHA256Crypt(std::to_string(protocol)))),
			SHA256Crypt(SHA256Crypt(game_version)),
			SHA256Crypt(SHA256Crypt(std::to_string(protocol)) + salts[3])
	};

	return SHA256Crypt(constant_values[0] + salts[0] + constant_values[1] + salts[1] + SHA256Crypt(MD5Crypt(SHA256Crypt(rid))) + salts[2] + constant_values[2]);
}*/
...
std::string GetRandomPart(const int& index)
{
	switch (index) 
	{
	    case 0:
